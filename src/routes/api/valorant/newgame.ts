...
export const POST: RequestHandler = async ({ request }) => {
	...
	try {
		const gamedata = await fetch(`https://api.battlefy.com/stages/${id}/matches`)
			.then((res) => res.json())
			.then((data) => {
				const result = BattlefyElementSchema.safeParse(data);
				if (!result.success) {
...
export const BattlefyElementSchema = z.object({
	_id: z.string(),
...
export type BattlefyElement = z.infer<typeof BattlefyElementSchema>;
